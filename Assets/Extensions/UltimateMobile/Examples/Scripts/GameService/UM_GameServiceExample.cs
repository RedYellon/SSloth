...
public class UM_GameServiceExample : BaseIOSFeaturePreview {
	...
	private string TEST_ACHIEVEMENT_1_ID = "AchievmentSample_1";
	private string TEST_ACHIEVEMENT_2_ID = "AchievmentSample_2";
	...
	void Awake() {
		...
		UM_GameServiceManager.instance.addEventListener(UM_GameServiceManager.PLAYER_CONNECTED, OnPlayerConnected);
		UM_GameServiceManager.instance.addEventListener(UM_GameServiceManager.PLAYER_DISCONNECTED, OnPlayerDisconnected);
	...
	void OnGUI() {
		...
		if(GUI.Button(new Rect(StartX, StartY, buttonWidth, buttonHeight), "Show Achievements")) {
			UM_GameServiceManager.instance.ShowAchivmentsUI();
		...
		if(GUI.Button(new Rect(StartX, StartY, buttonWidth, buttonHeight), "Report Achievements1")) {
			UM_GameServiceManager.instance.ReportAchievement(TEST_ACHIEVEMENT_1_ID);
			UM_ExampleStatusBar.text = "Achievment " + TEST_ACHIEVEMENT_1_ID  + " Reported";
		...
		if(GUI.Button(new Rect(StartX, StartY, buttonWidth, buttonHeight), "Increment Achievements2")) {
			...
			UM_ExampleStatusBar.text = "Achievment " + TEST_ACHIEVEMENT_2_ID  + " Oncremented by 20%";
