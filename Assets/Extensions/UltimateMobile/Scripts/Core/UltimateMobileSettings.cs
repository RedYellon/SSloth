...
public class UltimateMobileSettings : ScriptableObject {
	...
	public const string VERSION_NUMBER = "1.51";
		...
		get {
			if (instance == null) {
				...
				if (instance == null) {
					...
					#if UNITY_EDITOR
					string properPath = Path.Combine(Application.dataPath, UMSettingsPath);
					if (!Directory.Exists(properPath)) {
						AssetDatabase.CreateFolder("Extensions/", "UltimateMobile");
						AssetDatabase.CreateFolder("Extensions/UltimateMobile", "Resources");
					}
	...
	public void AddAchievment(UM_Achievement a) {
		Achievements.Add(a);
	}
	...
	public UM_Achievement GetAchievmentById(string id) {
	...
	public UM_Achievement GetAchievmentByIOSId(string id) {
		foreach(UM_Achievement a in Achievements) {
	...
	public UM_Achievement GetAchievmentByAndroidId(string id) {
		foreach(UM_Achievement a in Achievements) {
