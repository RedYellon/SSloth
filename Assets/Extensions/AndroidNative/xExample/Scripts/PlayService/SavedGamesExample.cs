...
public class SavedGamesExample : MonoBehaviour {
	...
	public void LoadSavedGames() {
		GooglePlaySavedGmaesManager.ActionAvaliableGameSavesLoaded += ActionAvaliableGameSavesLoaded;
		GooglePlaySavedGmaesManager.instance.LoadAvaliableSavedGames();

		SA_StatusBar.text = "Loading saved games.. ";
	}

	private void ActionAvaliableGameSavesLoaded (GooglePlayResult res) {

		GooglePlaySavedGmaesManager.ActionAvaliableGameSavesLoaded += ActionAvaliableGameSavesLoaded;
		if(res.isSuccess) {
			foreach(GP_SnapshotMeta meta in GooglePlaySavedGmaesManager.instance.AvaliableGameSaves) {
			...
			if(GooglePlaySavedGmaesManager.instance.AvaliableGameSaves.Count > 0) {
				GP_SnapshotMeta s =  GooglePlaySavedGmaesManager.instance.AvaliableGameSaves[0];
			...
			AndroidMessage.Create("Fail", "Avaliable Game Saves Load failed");
		}
	}
	...
	void OnSpanshotLoadDialogComplete (AndroidDialogResult res) {
		if(res == AndroidDialogResult.YES) {
			GP_SnapshotMeta s =  GooglePlaySavedGmaesManager.instance.AvaliableGameSaves[0];
			GooglePlaySavedGmaesManager.instance.LoadSpanShotByName(s.Title);
		}
	}
